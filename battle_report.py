...
TEXT_REPLACEMENTS = [
    ...
    ("current_precision_stability_v47_daily_auto_rolling_precision_rebuild", "目前精準穩定第47版：每日滾動精算重排"),
    ...
    ("industrial_v24_daily_auto_rolling_precision_rebuild", "工業級第24版：每日滾動精算重排"),
    ("actual_failure_feedback_rebuild", "實戰失準回灌重排"),
    ("daily_auto_rolling_precision_rebuild", "每日滾動精算重排"),
...
def compact_stability_governor_html(analysis):
    industrial = analysis.get("industrial_engine") or {}
    governor = industrial.get("stability_governor") or {}
    if not governor:
        return ""
    audit = governor.get("single_lock_audit") or {}
    locked = audit.get("locked_numbers") or []
    soft = audit.get("soft_guard_numbers") or []
    recent = audit.get("recent_single_rows") or []
    actions = governor.get("actions") or []

    action_rows = []
    for item in actions:
        kind = item.get("type")
        if kind == "single_lock_guard":
            action_name = "獨隻重複失準校正"
            detail = (
                f"原號碼 {fmt_numbers(item.get('removed', [])) or '-'}，"
                f"硬降權 {fmt_numbers(item.get('hard_removed', [])) or '-'}，"
                f"近期觀察降權 {fmt_numbers(item.get('soft_removed', [])) or '-'}，"
                f"改選 {fmt_numbers(item.get('replacement', [])) or '-'}"
            )
    ...
    for item in locked:
        locked_rows.append(
            "<tr>"
            f"<td>{fmt_numbers([item.get('number')])}</td>"
            f"<td>{item.get('recent8_count', 0)}</td>"
    ...
    if not locked_rows:
        locked_rows.append("<tr><td colspan='6'>沒有達到硬降權條件的獨隻。</td></tr>")
    ...
    if not soft_rows:
        soft_rows.append("<tr><td colspan='6'>沒有近期觀察降權號碼。</td></tr>")

    recent_rows = []
    for item in recent[:10]:
        recent_rows.append(
            "<tr>"
            f"<td>{escape_html(item.get('date', '-'))}</td>"
            f"<td>{fmt_numbers([item.get('number')])}</td>"
            f"<td>{item.get('hits', 0)}</td>"
            f"<td>{'命中' if item.get('passed') else '未命中'}</td>"
            "</tr>"
        )
    if not recent_rows:
        recent_rows.append("<tr><td colspan='4'>尚無獨隻結算資料。</td></tr>")

    return f"""
    <div class="band warn">
      <h2>穩定治理與錯誤修正紀錄</h2>
      <p>檢查時間：{escape_html(governor.get('checked_at', '-'))} / 狀態：{status_zh(governor.get('status'))}</p>
      <p>本次直接修正核心公式：低機率不再把上期失準號、最新開獎號、剛被打臉的高機率號當作加分；這些號碼會進入診斷與降權，不會被包裝成低機率。</p>
      <p>{escape_html(governor.get('policy', ''))}</p>
      <h3>本次修正動作</h3>
      <table><thead><tr><th>項目</th><th>處理內容</th></tr></thead><tbody>{''.join(action_rows)}</tbody></table>
      <h3>獨隻硬降權名單</h3>
      <table><thead><tr><th>號碼</th><th>近八期出現</th><th>近八期未中</th><th>累計命中 / 未中</th><th>命中率</th><th>原因</th></tr></thead><tbody>{''.join(locked_rows)}</tbody></table>
      <h3>獨隻近期觀察降權</h3>
      <table><thead><tr><th>號碼</th><th>近八期出現</th><th>近八期未中</th><th>累計命中 / 未中</th><th>命中率</th><th>原因</th></tr></thead><tbody>{''.join(soft_rows)}</tbody></table>
      <h3>最近獨隻結算</h3>
      <table><thead><tr><th>開獎日</th><th>獨隻</th><th>命中數</th><th>結果</th></tr></thead><tbody>{''.join(recent_rows)}</tbody></table>
    </div>
    """
...
<main>
  ...
  <section class="band update-box">
    <h2>\u7acb\u523b\u66f4\u65b0\u5230\u6700\u65b0</h2>
    <p class="update-note">\u96fb\u8166\u7248\u6309\u4e3b\u7a0b\u5f0f\u6700\u5916\u5c64\u300c\u7acb\u523b\u66f4\u65b0\u5230\u6700\u65b0.bat\u300d\uff1b\u624b\u52d5\u66f4\u65b0\u6703\u958b\u555f\u53ef\u8996\u5316\u9032\u5ea6\u8996\u7a97\uff0c\u4e0d\u518d\u96b1\u85cf\u57f7\u884c\u3002\u624b\u6a5f\u7248\u6309\u4e0b\u65b9\u5165\u53e3\u958b\u555f\u6700\u65b0\u540c\u6b65\u9801\u8207\u66f4\u65b0\u72c0\u614b\u3002</p>
    <p>
