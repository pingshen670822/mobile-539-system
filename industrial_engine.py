...
def super_single_candidate_decision(item, review=None):
    ...
    if repeated_failed_count >= 8:
        risk_penalty += 0.34
        risk_flags.append("\u8fd1\u671f\u91cd\u8907\u5931\u6e96\u904e\u9ad8")
...
def unlikely_number_analysis(draws, candidates, stability, review=None, limit=15, formula_lab=None):
    ...
    recall_numbers = {
        int(item.get("number"))
        for key in ["late_hit_numbers", "missed_actual_numbers", "monthly_recall_numbers"]
    ...
    for number in range(NUMBER_MIN, NUMBER_MAX + 1):
        ...
        evidence_score = 0.0
        risk_block_score = 0.0
        reasons = []
        rank = rank_map.get(number, 99)
        if number in previous_blocked:
            risk_block_score += 0.20
            reasons.append("\u8fd1\u671f\u9ad8\u6a5f\u7387\u5931\u6e96\u865f\u9694\u96e2\u6821\u6b63\uff0c\u4e0d\u76f4\u63a5\u5217\u4f4e\u6a5f\u7387")
        if number in failed:
            risk_block_score += 0.24
            reasons.append("\u4e0a\u671f\u5931\u6e96\u865f\u9700\u56de\u6e2c\u6821\u6b63\uff0c\u4e0d\u80fd\u53cd\u5411\u5145\u7576\u4f4e\u6a5f\u7387")
        ...
        if number in latest_set:
            if repeat_policy.get(number, {}).get("historical_support"):
                ...
                reasons.append("\u6700\u65b0\u958b\u734e\u865f\u9023\u838a\u9700\u7368\u7acb\u9a57\u7b97")
            else:
                risk_block_score += 0.30
                reasons.append("\u6700\u65b0\u958b\u734e\u865f\u9023\u838a\u672a\u901a\u904e\uff0c\u4e0d\u5217\u4f4e\u6a5f\u7387")
        ...
        if rank > 24:
            ...
            risk_block_score += 0.32
            reasons.append("\u5019\u9078\u524d\u6bb5\u865f\u4e0d\u53ef\u5217\u70ba\u4f4e\u6a5f\u7387")
        if repeated_failed.get(number, 0) >= 8:
