...
try {
  ...
  $ExtractCommand = '$ErrorActionPreference="Stop"; $root=$env:TW539_ROOT; $self=$env:TW539_SELF; $coreName=[string]::Concat([char]0x7cfb,[char]0x7d71,[char]0x6838,[char]0x5fc3); $core=Join-Path $root $coreName; if(-not (Test-Path -LiteralPath (Join-Path $core "main_one_click.ps1"))){ $marker="-----BEGIN_TW539_CORE_ZIP_BASE64-----"; $lines=[System.IO.File]::ReadAllLines($self,[System.Text.Encoding]::UTF8); $start=[Array]::IndexOf($lines,$marker); if($start -lt 0){ throw "Core payload missing." }; $payload=($lines[($start+1)..($lines.Length-1)] -join ""); $zip=Join-Path $root "tw539_core_payload.zip"; [System.IO.File]::WriteAllBytes($zip,[Convert]::FromBase64String($payload)); Expand-Archive -LiteralPath $zip -DestinationPath $root -Force; Remove-Item -LiteralPath $zip -Force }; $item=Get-Item -LiteralPath $core -Force; $item.Attributes=$item.Attributes -band (-bnot [System.IO.FileAttributes]::Hidden); Set-Location -LiteralPath $core; $env:TW539_VISIBLE_UPDATE="1"; & ($env:SystemRoot + "\System32\WindowsPowerShell\v1.0\powershell.exe") -NoProfile -ExecutionPolicy Bypass -File (Join-Path $core "main_one_click.ps1")'
